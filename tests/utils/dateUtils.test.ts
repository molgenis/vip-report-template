...
describe("formatDate", () => {
  ...
  it("formats date with single digit day and month", () => {
    const result = formatDate("2024-03-05T09:05:00");
    expect(result).toMatch(/^\d{2}-\d{2}-\d{4} \d{2}:\d{2}$/);
  });
  ...
  it("handles different years correctly", () => {
    expect(formatDate("2020-01-01T12:00:00")).toMatch(/^\d{2}-\d{2}-\d{4} \d{2}:\d{2}$/);
    expect(formatDate("2025-07-04T18:45:00")).toMatch(/^\d{2}-\d{2}-\d{4} \d{2}:\d{2}$/);
  });

  it("uses 24-hour format", () => {
    const result = formatDate("2024-01-15T14:00:00");
    expect(result).toMatch(/^\d{2}-\d{2}-\d{4} 14:00$/);

    const result2 = formatDate("2024-01-15T23:00:00");
