...
import { getRecordSample } from "./recordUtils";
...
function createConfigFieldCustomGenotype(sample: SampleContainer | null): ConfigCellCustom<CellValueGenotype> | null {
  ...
  return {
    type: "composed",
    id: "genotype",
    label: getSampleContainerLabel(sample),
    valueCount: () => 1,
...
function createConfigFieldCustomLocus(
  ...
  return {
    ...
    value: (record: Item<Record>): CellValueLocus => ({
      c: record.data.c,
      p: record.data.p,
      href: (sample ? `/samples/${sample.item.id}` : "") + `/variants/${variantType.id}/variant/${record.id}`,
    }),
  };
}
...
function createConfigFieldCustomVipC(
  ...
  return {
    ...
    value: (record: Item<Record>, recordContext: RecordContext): CellValueVipC => {
      ...
      return {
        href:
          (sample ? `/samples/${sample.item.id}` : "") +
          `/variants/${variantType.id}/variant/${record.id}/consequences/${valueIndex}`,
