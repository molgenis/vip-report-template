...
export const VariantsContainer: Component<{
  ...
  const onVariantTypeChange = (event: VariantTypeChangeEvent) => {
    navigate(props.sample !== null ? `/samples/${props.sample.item.id}/variants/${event.id}` : `/variants/${event.id}`);
  };
    ...
    <>
      ...
      <div class="columns is-1">
        <div class="column is-2-fullhd is-3">
          <Show when={props.variantTypeIds.size > 1}>
            <div class="columns">
              <div class="column">
                <VariantTypeSelect
                  value={props.variantType}
                  variantTypeIds={props.variantTypeIds}
