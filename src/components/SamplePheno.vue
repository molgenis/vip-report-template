<template>
  <div
    v-if="
      phenotypes.items.length === 1 &&
        phenotypes.items[0].phenotypicFeaturesList.length > 0
    "
  >
    <span
      v-for="(phenotypicFeature, index) in phenotypes.items[0]
        ...
        <Anchor
          ...
          :href="
            'https://hpo.jax.org/app/browse/term/' +
              encodeURIComponent(phenotypicFeature.type.id)
          "
...
export default Vue.extend({
  ...
  computed: {
    hpoTerms: function() {
      const hpoTerms = [];
      for (const phenotypicFeature of this.phenotypes.items[0]
        .phenotypicFeaturesList) {
